...
func NewMigrateToCpswapInstructionBuilder() *MigrateToCpswap {
	...
	nd.AccountMetaSlice[4] = ag_solanago.Meta(RaydiumCPMMProgram)
	nd.AccountMetaSlice[13] = ag_solanago.Meta(LockProgram)
	nd.AccountMetaSlice[16] = ag_solanago.Meta(AuthorityPDA).WRITE()
	nd.AccountMetaSlice[22] = ag_solanago.Meta(TokenProgram)
	nd.AccountMetaSlice[23] = ag_solanago.Meta(TokenProgram)
	nd.AccountMetaSlice[24] = ag_solanago.Meta(AssociatedTokenProgram)
	nd.AccountMetaSlice[25] = ag_solanago.Meta(SystemProgram)
	nd.AccountMetaSlice[26] = ag_solanago.Meta(RentProgram)
	nd.AccountMetaSlice[27] = ag_solanago.Meta(TokenMetadataProgram)
	return nd
}
...
func (inst *MigrateToCpswap) SetAccounts(accounts []*ag_solanago.AccountMeta) error {
	inst.AccountMetaSlice = accounts
	return inst.Validate()
}

func (inst *MigrateToCpswap) SetRemainingAccounts(metas []*ag_solanago.AccountMeta) *MigrateToCpswap {
	inst.AccountMetaSlice = append(inst.AccountMetaSlice[0:28], metas...)
	return inst
}

func (inst *MigrateToCpswap) GetRemainingAccounts() []*ag_solanago.AccountMeta {
...
func NewMigrateToCpswapInstruction(
	...
	poolLpToken ag_solanago.PublicKey) *MigrateToCpswap {
...
// NewSimpleMigrateToCpswapInstruction declares a new MigrateToCpswap instruction with the provided parameters and accounts.
func NewSimpleMigrateToCpswapInstruction(
	// Accounts:
	payer ag_solanago.PublicKey,
	baseMint ag_solanago.PublicKey,
	quoteMint ag_solanago.PublicKey,
	...
	quoteVault ag_solanago.PublicKey,
	poolLpToken ag_solanago.PublicKey) *MigrateToCpswap {
	poolState := MustFindPoolStateAddress(baseMint, quoteMint)
	return NewMigrateToCpswapInstructionBuilder().
		SetPayerAccount(payer).
		SetBaseMintAccount(baseMint).
		SetQuoteMintAccount(quoteMint).
