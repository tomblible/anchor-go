...
type PermissionlessFarmSwitchAccount struct {
	// Bump to identify PDA
	Bump uint8
...
var PermissionlessFarmSwitchAccountDiscriminator = [8]byte{37, 92, 147, 78, 250, 100, 147, 137}

func (obj PermissionlessFarmSwitchAccount) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	// Write account discriminator:
	err = encoder.WriteBytes(PermissionlessFarmSwitchAccountDiscriminator[:], false)
	if err != nil {
		return err
	}
	// Serialize `Bump` param:
...
func (obj *PermissionlessFarmSwitchAccount) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	// Read and check account discriminator:
	{
		discriminator, err := decoder.ReadTypeID()
		if err != nil {
		...
		if !discriminator.Equal(PermissionlessFarmSwitchAccountDiscriminator[:]) {
			return fmt.Errorf(
				"wrong discriminator: wanted %s, got %s",
